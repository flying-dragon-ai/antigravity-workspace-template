"""Integration tests for rb report and rb log-decision commands."""
...
def test_report_creates_file(tmp_path: Path) -> None:
    """rb report writes a timestamped entry to reports.md."""
    ...
    reports = tmp_path / ".repobrain" / "memory" / "reports.md"
...
def test_log_decision_creates_file(tmp_path: Path) -> None:
    """rb log-decision writes a decision with rationale."""
    result = runner.invoke(
        app,
        [
            "log-decision",
    ...
    log = tmp_path / ".repobrain" / "decisions" / "log.md"
    assert log.exists()
