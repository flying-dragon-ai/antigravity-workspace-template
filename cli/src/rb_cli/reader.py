"""Pure-pathlib reader for .repobrain/ context files. No engine dependency."""
...
def read_repobrain_context(workspace: Path) -> dict[str, str]:
    """Read all context files from .repobrain/ directory.
    ...
    for md_file in sorted(rb_dir.glob("*.md")):
        try:
...
def append_decision(workspace: Path, decision: str, rationale: str) -> Path:
    ...
    """
    decisions_dir = workspace / ".repobrain" / "decisions"
