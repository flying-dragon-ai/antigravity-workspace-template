...
RepoBrain is a portable, evidence-grounded repository knowledge engine. Its
primary workflow is:

- `rb-refresh` scans a workspace and builds `.repobrain/` knowledge artifacts.
- `rb-ask` routes codebase questions to the relevant module context and answers
  with source evidence.
- Native plugins, raw CLI entrypoints, context files, and `rb-mcp` expose the
...
- **Integration:** Model Context Protocol (MCP) for optional server and consumer
  flows.
- **CLI:** `typer` + `rich` for `rb` and engine entrypoints.
- **Testing:** Pytest.
...
- The core product boundary is the knowledge workflow: refresh, ask, and the
  `.repobrain/` artifacts they produce.
- Local tools and MCP tools should be explicit delivery surfaces with documented
  permissions.
- Generated state belongs under `.repobrain/`, `memory/`, or `artifacts/` as
  appropriate; do not hide persistent behavior in unrelated paths.
- `rb init` / `agent-repo-init` is for scaffolding new repositories; it is not
  required before `rb-refresh` on an existing repository.
...
- **Hub module** (`engine/repobrain_engine/hub/`): scans the workspace,
  generates conventions and module knowledge, builds routing indexes, and answers
  project questions via LLM.
- **CLI commands:** `rb-refresh` builds or refreshes `.repobrain/`; `rb-ask`
  answers grounded project questions; `rb-mcp` exposes `ask_project` and
  `refresh_project` to MCP hosts. The wrapper `rb refresh` / `rb ask` commands
...
- `RB_RETRIEVAL_MODE=compact` is the default. `full` keeps richer artifacts;
...
- External MCP server consumption requires explicit opt-in via `MCP_ENABLED=true`
  and `RB_ALLOW_MCP=true`. Stdio MCP servers inherit the process environment plus
