...
The system SHALL route `rb ask` and `rb refresh` through published engine
entrypoints rather than unpublished legacy binaries.

#### Scenario: Ask through the CLI package
- **WHEN** a user runs `rb ask "question" --workspace /path/to/project`
...
- **WHEN** a user runs `rb ask ...` or `rb refresh ...` without the engine
  package installed
...
- **WHEN** a developer runs
  `python -m repobrain_engine ask "question" --workspace /path/to/project`
- **THEN** the engine dispatches to the ask pipeline
- **AND** the behavior matches `rb-ask`

#### Scenario: Execute refresh via hub module invocation
- **WHEN** a developer runs
  `python -m repobrain_engine.hub refresh --workspace /path/to/project`
- **THEN** the engine dispatches to the refresh pipeline
- **AND** the behavior matches `rb-refresh`

#### Scenario: Execute MCP via module invocation
- **WHEN** a developer runs
  `python -m repobrain_engine.hub mcp --workspace /path/to/project`
- **THEN** the engine starts the knowledge-hub MCP server
- **AND** the behavior matches `rb-mcp`
