...
def test_existing_old_rb_mcp_triggers_pipx_upgrade(tmp_path: Path, monkeypatch) -> None:
    hook = _load_hook_module()
    ...
    monkeypatch.setattr(hook, "has", lambda cmd: cmd in {"rb-mcp", "pipx"})
...
def test_missing_rb_mcp_uses_first_install_path(tmp_path: Path, monkeypatch) -> None:
    hook = _load_hook_module()
    plugin_root = tmp_path / "plugin"
    engine_dir = plugin_root / "engine"
    (plugin_root / "cli").mkdir(parents=True)
...
def test_existing_matching_rb_mcp_skips_install(tmp_path: Path, monkeypatch) -> None:
    hook = _load_hook_module()
    plugin_root = tmp_path / "plugin"
    engine_dir = plugin_root / "engine"
    engine_dir.mkdir(parents=True)
    ...
    monkeypatch.setattr(hook, "prepend_path", lambda path: None)
    monkeypatch.setattr(hook, "user_scripts_bin", lambda: None)
    monkeypatch.setattr(hook, "has", lambda cmd: cmd == "rb-mcp")
