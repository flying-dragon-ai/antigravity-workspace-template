...
def _run_codex_host_runner_sync(
    *,
    ...
    timeout = _coerce_float(
        timeout_seconds if timeout_seconds is not None else os.environ.get("RB_HOST_TIMEOUT_SECONDS"),
        DEFAULT_HOST_TIMEOUT_SECONDS,
    )
    max_chars = _coerce_int(
        max_context_chars if max_context_chars is not None else os.environ.get("RB_HOST_MAX_CONTEXT_CHARS"),
...
def _build_codex_prompt(
    *,
    ...
    sections = [
        (
            "You are RepoBrain's local Codex host runner for read-only codebase Q&A.\n"
