...
def _verbose() -> bool:
    ...
    return os.environ.get("RB_SKILLS_VERBOSE", "0").strip().lower() in {"1", "true", "yes"}


def load_skills(agent_tools: dict[str, Callable[..., Any]]) -> str:
    """
    Scans repobrain_engine/skills/ directory for skill packages.

    For each subfolder in repobrain_engine/skills/:
    ...
    # Iterate over directories in repobrain_engine/skills/
        ...
        if tools_file.exists():
            try:
                spec = importlib.util.spec_from_file_location(
                    f"repobrain_engine.skills.{skill_name}.tools", tools_file
                )
